...
enum Message {
	 ...
	 * @param s вЂ” array of any object that you need to print out.
	 * Example:
	...
	public static void init(PluginBase plg){
		...
		languageSave = plg.getConfig().getNested("general.language-save",true);
		plg.getConfig().setNested("general.language-save",languageSave);
		debugMode = plg.getConfig().getNested("general.debug-mode",false);
		plg.getConfig().setNested("general.debug-mode",debugMode);
		plg.saveConfig();
		initMessages();
		if (languageSave) saveMessages();
